...
|---|---|---|---|
| **Display / Titles** ("EDGECASE", screen titles) | **Caesar Dressing** | `caesar_dressing.ttf` | Literally designed to mimic ancient Greek carved lettering — blocky, angular A/E/S with chisel notches. Instant "temple inscription". |
| **Buttons / sub-headers** | **Cinzel Black** (weight 900 of Cinzel) | `cinzel_black.ttf` | Roman-inscription capitals, monumental at 900 weight; more legible than Caesar Dressing at button sizes. |
| **Body / captions** (optional, phase 2) | **EB Garamond** or keep `serif` | `eb_garamond.ttf` | Quiet classical body face; low priority. |

Steps:
1. `mkdir -p app/src/main/res/font`
2. Download each family from Google Fonts → copy the static TTF in, renamed as above.
3. Reference as `@font/caesar_dressing` etc. (minSdk 30 → the `res/font` XML API is fully
   supported, no support-lib fallback needed).

**Fallback option (no binary assets):** Downloadable Fonts via Google Play services
(`androidx.core:core:…`, `FontsContractCompat`, `res/font/*.xml` with `<font-family
app:fontProviderAuthority="com.google.android.gms.fonts" …>`). Works, but adds a first-launch
network fetch and a flash of fallback font — **bundling is recommended** for an app whose whole
identity is the title lettering.

### 4.3 The Greek-glyph title trick (optional, highly thematic)

For the main title ONLY, the string can use genuine Greek codepoints that read as Latin:
...
With Caesar Dressing these render as massive chiseled bars. **Caveats:** screen readers will
pronounce it strangely — if used, add `android:contentDescription="EdgeCase"` on the title
TextView; and keep the launcher label plain "EdgeCase". This is a stylistic option — decide at
implementation time; the header spec below works with either string.
...
<style name="TitleMonolith" parent="TextAppearance.AppCompat">
    <item name="android:fontFamily">@font/caesar_dressing</item>
...
Keep `EngravedHeader`, `BodySerif`, `CaptionSerif`, `StoneButtonText` in the file (legacy
consumers like the Customize dialog can migrate in phase 2) but **all three screen layouts move
to the new styles.**
...
|---|---|---|
| `layout_screen_main_menu.xml` | lines 44–63 (LinearLayout + left-aligned 18sp TextView) | `EDGECASE` (or `ΞDGΞCΛSΞ`, §4.3) @ 26sp |
...
// Main-menu copy keeps the default "EDGECASE" @ header_title_size — no code needed.
```
...
`bg_stone_texture.xml` stays in the repo (the Customize dialog panel still uses it until phase 2)
but no screen references it after this change.
...
| `seed` | fixed | any Long | fixed = same temple every launch (recommended — the room feels *real*); `System.nanoTime()` = new cave each launch |
| `maxGlowAlpha` | 0.55 | 0.3–0.7 | ambience vs. readability; drop to 0.45 if text contrast suffers on test devices |
...
> on taller buttons (bevel just sits 48dp down). If pixel-perfect on all heights is wanted,
> switch layers 3–4 to a 90° `<gradient>` with a hard stop — acceptable substitute.
...
| `ic_silver_ring.xml` (oval) | used in header (**delete from header**, §5.5) and in `layout_item_shortcut_tile.xml` icon frame | Header: removed. Tile: replace the oval ring with a square "socket" drawable (§10.3) — ovals are rounded by definition |
| SeekBar hue track `GradientDrawable` (`SliverCustomizeDialog.paintHueTrack`, `cornerRadius = 8f * density`) | rounded | `g.cornerRadius = 0f` |
| `LabeledSeekBar` system thumb | round thumb | acceptable (system affordance) in phase 1; phase 2 option: custom square-gem thumb drawable |
...
Used by: Customize dialog window, Discard dialog window, and (`phase 2`)
`dialog_customize_sliver.xml` root.
...
`match_parent`, 1 unit ≈ 1dp on a ~360dp-wide content column — the gap lands at ≈40dp. If an
exact 40dp on every screen width is ever required, the alternative is a centered fixed-width
(120dp) fang-pair ImageView overlaid on a full-width line view — visually identical, not worth
the extra layout in phase 1.
...
### 10.5 Checkboxes (phase 2, optional)
...
`layout_screen_main_menu.xml` bottom-left version TextView: text `v1.3.5` →
**`Ε΄ · ΕΚΔΟΣΙΣ 1.4.0`** ("edition" in Greek) or simply `MCCCXL` style is Roman — stay Greek:
`ΕΚΔ. 1.4.0`. Style stays `CaptionSerif` (or new `CaptionChiseled`). Pure flavor, zero risk.
...
- The side flip happens the moment `sliverSide` is finalized at snap-animation end: the arrow
  hops flanks in the same frame the fang mirrors — reads as intentional. *Optional refinement:*
  flip mid-flight by deriving `dir` from `sliverPixelX < (mockupLeft + mockupRight) / 2f`
  instead of `sliverSide` inside both arrow functions.
...
|---|---|---|---|---|
| 1 | `app/src/main/res/font/caesar_dressing.ttf` | binary | display font | §4.2 |
...
| 11 | *(phase 2)* `selector_gem_checkbox.xml` + 2 state vectors | drawable | gem checkboxes | §10.5 |
| 12 | *(phase 2, optional)* `eb_garamond.ttf` | binary | body font | §4.2 |
...
| `res/layout/dialog_customize_sliver.xml` | section dividers → `ic_divider_fangs` @ 12dp (§10.4); *(phase 2)* root bg → `bg_temple_panel` |
| `res/layout/layout_item_shortcut_tile.xml` | ring → socket (§10.3) |
| `MainActivity.kt` | scoped header-title assignment (§5.5); discard-dialog square bg (§9); back-gesture migration to `OnBackPressedDispatcher` (§12.4); *(tier-1 #5)* microcopy |
...
| `SliverCustomizeDialog.kt` | window bg → `bg_temple_panel`; hue-track `cornerRadius = 0f` (§9) |
...
| `OnBackPressedDispatcher` + `OnBackPressedCallback` (androidx.activity via appcompat) | §12.4 back-gesture navigation |
...
- [ ] Add fonts to `res/font/` (verify licenses archived in `Docs/`).
...
### Phase 5 — De-rounding & secondary *(~1 h)*
- [ ] §9 audit table end-to-end (panels, PositioningView radii, both dialogs, hue track, socket, divider, version label).
